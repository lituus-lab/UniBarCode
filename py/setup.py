...
"""Build unibarcode._core, a Cython extension over the UniBarCode C ABI.
...
PKG_DIR = os.path.join(HERE, "unibarcode")
VENDOR_DIR = os.path.join(HERE, "_nimsrc")
NIMBLE_FILE = "UniBarCode.nimble"
...
if sys.platform == "win32":
    LIB_NAME, BUNDLED = "UniBarCode.lib", False
    LINK_ARGS, NIMBLE_TASK = [], "clibMsvc"
elif sys.platform == "darwin":
    LIB_NAME, BUNDLED = "libUniBarCode.dylib", True
    LINK_ARGS, NIMBLE_TASK = ["-Wl,-rpath,@loader_path"], "clib"
else:
    LIB_NAME, BUNDLED = "libUniBarCode.so", True
...
def nim_project_dir():
    """Where UniBarCode.nimble lives: the real repo root in a normal
...
def ensure_lib_built():
    ...
    try:
        ...
        raise SystemExit(
            "setup.py: `nimble` not found on PATH. Building unibarcode from "
...
# The sdist ships the pre-transpiled unibarcode/_core.c, not the .pyx (Cython
...
# needing no Cython.
pyx = os.path.join("unibarcode", "_core.pyx")
ext = Extension(
    "unibarcode._core",
    sources=[pyx if os.path.exists(os.path.join(HERE, pyx)) else os.path.join("unibarcode", "_core.c")],
    include_dirs=[INCLUDE],
    library_dirs=[LIB_DIR],
    extra_link_args=LINK_ARGS,
    libraries=["UniBarCode"],
)
...
setup(
    ...
    package_data={"unibarcode": [LIB_NAME] if BUNDLED else []},
    exclude_package_data={"unibarcode": ["_core.c"]},
    zip_safe=False,
)
