...
```bash
nimble install https://github.com/lituus-lab/UniBarCode    # Nim
pip install lituus-unibarcode                              # Python
```

For C, the build produces `libUniBarCode.a` and `include/UniBarCode.h`:

```bash
build/unigate clibStatic
cc -Iinclude your.c libUniBarCode.a
```

The PyPI distribution is `lituus-unibarcode`; the import name stays
`unibarcode`. Those are two decisions, and the bare names are not all
available.
