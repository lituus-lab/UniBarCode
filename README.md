...
```text
src/UniBarCode.nim          umbrella module
src/UniBarCode/fibonacci.nim  Nim core (NimContracts)
src/UniBarCode/c_api.nim    C ABI
...
|---|---|---|
| `UniBarCode` | `UniFoo` | `UniAccurate`, `UniMath` |
| `unibarcode` | `unifoo` | `uniaccurate`, `unimath` |
| `libUniBarCode` | `libUniFoo` | `libUniAccurate` |
| `UniBarCode.h` | `UniFoo.h` | `UniAccurate.h` |
| `lituus-unibarcode` | `lituus-unifoo` | `lituus-uniaccurate` |

The C symbol prefix is the library's own name in lower case —
`unibarcode_fibonacci`, so `unifoo_*`. Short prefixes read better and collide:
a binary that links several engines at once holds them all in one namespace.

Files to rename: `UniBarCode.nimble`, `src/UniBarCode.nim`, `src/UniBarCode/`,
`include/UniBarCode.h`, `tests/c/test_unibarcode.c` (+ its Makefile target),
`py/unibarcode/`. Then update `LICENSE`/`NOTICE` copyright and the ADR titles.
